...
export default class Renderer {
    ...
    private createStaticDrawBuffer(vertices: Float32Array) {
        const gl = this.gl;
    ...
    private createIBO(indices: Uint16Array) {
        const gl = this.gl;
    ...
    setBufferData(buffer: WebGLBuffer, data: ArrayBufferView | ArrayBuffer) {
        const gl = this.gl;
        gl.bindBuffer(gl.ARRAY_BUFFER, buffer);
    ...
    updateGeometryVertices(geometry: Geometry) {
        if (geometry.dynamic) {
            if (geometry.vao) {
                for (let i = 0, len = geometry.attributes.length; i < len; ++i) {
                    const attribute = geometry.attributes[i];
                    attribute.vbo && this.setBufferData(attribute.vbo, attribute.vertices);
                }
            }
        } else {
    ...
    resizeTexture(texture: Texture, width: number = this.state.width, height: number = this.state.height) {
        if (width < 0 || height < 0) {
            throw new Error('Negative width/height');
        }
    ...
    resizeFrameBufferTextures(frameBuffer: FrameBuffer, width: number = this.state.width, height: number = this.state.height) {
        for (let texture of frameBuffer.textures) {
    ...
    bindFramebuffer(frameBuffer: FrameBuffer | null = null) {
    ...
    drawGeometry(geometry: Geometry) {
        ...
        if (!geometry.vao) {
            ...
            const attribLocations = this.attribLocations;
            for (let geoAttr of geometry.attributes) {
                if (!geoAttr.vbo) {
                    geoAttr.vbo = geometry.dynamic ? this.createDynamicDrawBuffer(geoAttr.vertices) : this.createStaticDrawBuffer(geoAttr.vertices);
                }
    ...
    private clientWaitAsync(sync: WebGLSync, flags: number = 0) {
        const gl = this.gl;
            ...
            function check() {
                ...
                switch (res) {
                    ...
                    case gl.TIMEOUT_EXPIRED:
                        return requestIdleCallback(check);
                }
                resolve();
            }
    ...
    async flushAsync() {
        ...
        if (!sync) {
            throw new Error('Failed to create WebGLSync object');
        }
        gl.flush();
        await this.clientWaitAsync(sync);
        gl.deleteSync(sync);
    }

    async readRgbaAsync(out: Uint8Array,
                        x: number = 0,
                        y: number = 0,
                        w: number = this.state.width,
                        h: number = this.state.height,
    ) {
        const gl = this.gl;
        const buffer = gl.createBuffer();
        if (buffer == null) {
        ...
        gl.bufferData(gl.PIXEL_PACK_BUFFER, out, gl.STREAM_READ);
