...
import {getVoxel, setVoxel, Voxels} from '../../utils/voxel/data';
...
import icon from './voxel-range-brush.png';
...
export default class VoxelRangeBrush extends SkeletonEngineTool {

    name = VoxelRangeBrush.name;
    label = 'Range Brush';
    icon = icon;
    ...
    private readonly isVoxelExists: (x: number, y: number, z: number) => boolean = function (this: VoxelRangeBrush, x: number, y: number, z: number) {
    ...
    onPostSolve(engine: SkeletonEngine): void {
        ...
        if (engine.input.mouseLeft || engine.input.mouseRight) {
            ...
            for (let x = x0; x <= x1; ++x) {
                for (let y = y0; y <= y1; ++y) {
                    for (let z = z0; z <= z1; ++z) {
                        if (getVoxel(voxelData.value, x, y, z) != null) {
